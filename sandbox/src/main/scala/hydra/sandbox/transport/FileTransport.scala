        ...
        .map { flow =>
          ...
          f.onComplete {
            case Success(_) =>
              ...
              val md = FileRecordMetadata(
                destinations(r.destination),
                0,
                ackStrategy = r.ackStrategy
              )
          ...
          callback.onCompletion(
            ...
            Some(
              new IllegalArgumentException(
                s"File stream ${r.destination} not found."
              )
            )
          )
        )
...
object FileTransport extends ConfigSupport {
  ...
  def props(destinations: Config): Props = {
    val map = ConfigSupport.toMap(
      destinations.getConfig("hydra.transports.file.destinations")
    )
