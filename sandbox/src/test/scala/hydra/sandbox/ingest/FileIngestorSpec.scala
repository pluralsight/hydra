...
class FileIngestorSpec
  ...
  val fileProducer =
    system.actorOf(Props(new ForwardActor(transportProbe.ref)), "file_producer")
  ...
  implicit override val patienceConfig =
    PatienceConfig(
      ...
      interval = scaled(Span(60, Millis))
    )
  ...
  describe("The FileIngestor") {
    ...
    it("validates") {
      val hr =
        HydraRequest("0", "test").withMetadata("hydra-file-stream" -> "test")
      ingestor ! Validate(hr)
      whenReady(FileRecordFactory.build(hr)) { r => expectMsg(ValidRequest(r)) }
      val hr1 =
        HydraRequest("0", "test").withMetadata("hydra-file-stream" -> "unknown")
    ...
    it("transports") {
      ...
      whenReady(FileRecordFactory.build(hr)) { r =>
        ...
        transportProbe.expectMsg(
          10.seconds,
          Produce(
            FileRecord("test", "test", AckStrategy.NoAck),
            self,
            AckStrategy.NoAck
          )
        )
