...
class KafkaClientAlgebraSpec
  ...
  private def avroTests(schemaRegistry: SchemaRegistry[IO], kafkaClient: KafkaClientAlgebra[IO], shouldCommitOffsets: Boolean): Unit = {
    ...
    "consume avro messages from kafka and skip incorrect message" in {
      ...
      val records = kafkaClient.consumeMessages(topic11,"newConsumerGroup", shouldCommitOffsets).take(2).compile.toList.unsafeRunSync()
      records should have length 2
      records.head shouldBe (key1, Some(value1), None)
      records.tail.head shouldBe (key, Some(value), None)
    }
