...
/**
  ...
  */
case class KafkaRecordMetadata(offset: Long, timestamp: Long, destination: String, partition: Int,
                               deliveryId: Long, ackStrategy: AckStrategy) extends RecordMetadata

object KafkaRecordMetadata extends DefaultJsonProtocol {
  def apply(kmd: org.apache.kafka.clients.producer.RecordMetadata, deliveryId: Long, ackStrategy: AckStrategy): KafkaRecordMetadata = {
  ...
  implicit object AckStrategyFormat extends RootJsonFormat[AckStrategy] {
    ...
    override def read(json: JsValue): AckStrategy = AckStrategy.apply(json.convertTo[String]).getOrElse(AckStrategy.NoAck)
  }
