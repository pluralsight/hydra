  ...
  override def transport = {
    case Deliver(record: JdbcRecord, deliveryId, callback) =>
      Try {
        ...
        val op = Option(record.payload)
          .map(p => Upsert(p))
          .getOrElse(DeleteByKey(record.keyValues))
        writer.execute(op)
        callback.onCompletion(
          ...
          Some(
            JdbcRecordMetadata(
              record.destination,
              ackStrategy = record.ackStrategy
            )
          ),
          None
        )
  ...
  private[jdbc] def getOrUpdateWriter(db: DbProfile, rec: JdbcRecord) = {
    ...
    writers.getOrElseUpdate(
      ...
      new JdbcRecordWriter(
        db.settings,
        db.provider,
        SchemaWrapper.from(schema, rec.primaryKeys),
        SaveMode.Append,
    ...
    applicationConfig
      ...
      .map { cfg =>
        cfg.root().entrySet().asScala.foreach { e =>
          Try {
            val props = ConfigSupport
              .toMap(e.getValue.asInstanceOf[ConfigObject].toConfig)
            val settings =
              JdbcWriterSettings(e.getValue.asInstanceOf[ConfigObject].toConfig)
            dbProfiles.put(e.getKey, new DbProfile(e.getKey, props, settings))
          }.recover {
            case ex =>
              log.error(s"Unable to load db profile ${e.getKey()}.", ex)
          }
        }
      }
