...
class JdbcRecordFactorySpec
  ...
  val schemaPK =
    new Schema.Parser().parse(Source.fromResource("schemaPK.avsc").mkString)
  ...
  override implicit val patienceConfig =
    PatienceConfig(timeout = scaled(2000 millis), interval = scaled(100 millis))
  ...
  describe("The JDBC record factory") {
    it("extracts primary keys if present") {
      ...
      val req2 = HydraRequest("1", "test").withMetadata(
        JdbcRecordFactory.PRIMARY_KEY_PARAM -> "id"
      )
      JdbcRecordFactory.pk(req2, schemaNPK) shouldBe Seq("id")

      val req3 = HydraRequest("1", "test").withMetadata(
        JdbcRecordFactory.PRIMARY_KEY_PARAM -> "unknown"
      )
    ...
    it("throws an error if no schema is in the request metadata") {
      val req = HydraRequest("1", "test")
      whenReady(factory.build(req).failed)(
    ...
    it("throws an error if payload if validation is strict") {
      val request =
        HydraRequest("123", """{"id":1, "field":2, "name":"test"}""")
          .withMetadata(HYDRA_SCHEMA_PARAM -> "jdbc-test")
      whenReady(factory.build(request).failed)(
        _ shouldBe a[JsonToAvroConversionExceptionWithMetadata]
      )
      ...
      val request =
        HydraRequest("123", """{"id":1, "name":"test", "rank" : 1}""")
          .withMetadata(
            ...
            JdbcRecordFactory.DB_PROFILE_PARAM -> "table"
          )

      whenReady(factory.build(request))(
        _.destination shouldBe schemaNPK.getName
      )
    ...
    it("builds a record without a PK") {
      ...
      whenReady(factory.build(request)) { rec =>
        ...
        rec.payload shouldBe new JsonConverter[GenericRecord](schemaNPK)
          .convert("""{"id":1, "name":"test", "rank" : 1}""")
      }
    }
    ...
    it("builds a record with a PK") {
      ...
      whenReady(factory.build(request)) { rec =>
        ...
        rec.payload shouldBe new JsonConverter[GenericRecord](schemaPK)
          .convert("""{"id":1, "name":"test", "rank" : 1}""")
      }
    }
  }
