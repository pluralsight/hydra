...
/**
  ...
  */
class ReflectionsWrapperSpec extends Matchers with FlatSpecLike with BeforeAndAfterAll {
  ...
  it should "load by subtype" in {
    ReflectionsWrapper.reflections.getSubTypesOf(classOf[ServiceProvider]) should contain(classOf[TestServiceProvider])
  }
}