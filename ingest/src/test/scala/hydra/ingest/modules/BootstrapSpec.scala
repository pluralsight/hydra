...
class BootstrapSpec extends AnyWordSpecLike with Matchers with NotificationsTestSuite {
  ...
  private final class TestKafkaClientAlgebraWithPublishTo(publishTo: Ref[IO, List[(GenericRecord, Option[GenericRecord], Option[Headers])]]
    ...
    override def streamStringKeyFromGivenPartitionAndOffset(topicName: TopicName, consumerGroup: ConsumerGroup, commitOffsets: Boolean, topicPartitionAndOffsets: List[(TopicPartition, Offset)]): fs2.Stream[IO, ((Option[String], Option[GenericRecord], Option[Headers]), (Partition, Offset), Timestamp)] = fs2.Stream.empty

    override def streamAvroKeyFromGivenPartitionAndOffset(topicName: TopicName, consumerGroup: ConsumerGroup, commitOffsets: Boolean, topicPartitionAndOffsets: List[(TopicPartition, Offset)]): fs2.Stream[IO, ((GenericRecord, Option[GenericRecord], Option[Headers]), (Partition, Offset), Timestamp)] = fs2.Stream.empty
