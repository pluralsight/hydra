...
class HydraIngestorRegistryClient(registryPath: String)(implicit val system: ActorSystem) {
  ...
  def lookupIngestor(name: String)(implicit timeout: Timeout): Future[LookupResult] = {
...
object HydraIngestorRegistryClient {
  ...
  def registryPath(config: Config) = config.get[String]("ingest.ingestor-registry.path")
    .valueOrElse(s"/user/service/${ActorUtils.actorName(classOf[IngestorRegistry])}")

  def apply(config: Config)(implicit system: ActorSystem): HydraIngestorRegistryClient = {
